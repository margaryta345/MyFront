...
const Test6 = ({ module, name }) => {
  ...
  const onButtonClick = async () => {
    ...
    setScore(calculatedScore); // Оновлюємо стан score при клікі на кнопку "Відправити"
    const data = await dispatch(
      fetchDoneSend({ index: 34, value: calculatedScore })
    );
  ...
  const calculatePercentage = () => {
    if (questions.length === 0) return 0; // Якщо немає питань, результат - 0%
  ...
  const resetTest = () => {
    ...
    setBut(false);
    // Додаткові дії для підготовки тесту до повторного проходження (якщо потрібно)
  };
        ...
        {!done ? (
          ...
          <div className="flex border-t border-b pt-8 pb-8">
            ...
            <div className="flex items-center pl-10 ">
              ...
              <div>
                <Link
                  to={`/lessons/66389aff0dd35f20b0dc1f12`}
