...
const ConsoleLesson1 = () => {
  ...
  const runCode = async () => {
    ...
    try {
      axios
        .post(`/task1`, { requestData: sourseCode })
        .then((res) => {
          console.log(res);
  ...
  return (
    <div>
      <div className="lg:text-base  text-gray-600 items-center mt-6 mb-6 flex">
        ...
        <p className="ml-2">Задача типи даних,оператори,методи рядків</p>
      </div>
      <div className="flex">
        <div className="w-1/2">
          <div className="bg-[#252526] border border-gray-500">
            ...
            <Editor
              ...
              defaultValue={`def create_variables():
     # тут писати код для задачі 1
              
     #нічого не міняємо
     return [integer_var, float_var, string_var, list_var, tuple_var, set_var, dict_var, bool_var]
         
     
def check_admission_eligibility(age, math,language,history):
...
def process_string(text):
     ...
     #нічого не міняємо
     return [modified_text, words, joined_text]`}
              onMount={onMount}
              value={value}
